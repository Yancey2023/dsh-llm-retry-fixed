 ...
 * through the OFFICIAL session-projection API (`ctx.sessionProjections`
 * with the same fold semantics as the stock policy's `llmRetry` unit, under
 * our own key `llmRetryFixed` — durable, restart-safe, and derived from the
 * session log), and every non-matched failure (SERVER / TIMEOUT / TRANSPORT /
 * …) is passed through untouched to the stock policy.
 *
...
/** The projection key under which this executor's retry count is derived. */
export const PROJECTION_KEY = 'llmRetryFixed'

export function policyKey(codes, delays) {
  return JSON.stringify(['fixed', [...codes].sort(), delays])
}

function retryStateKey(provider, key) {
  return JSON.stringify([provider, key])
}
...
export function apply(ctx, config = {}) {
  ...
  // Official session-projection unit (the API the current DSH exposes for
  // exactly this — deriving retry state from the session log). Shares the
  // stock policy's fold semantics, under our own key, so fixed-schedule
  // retries have their own durable count and the two policies never collide
  // (the state key embeds the policy key, which differs between them).
  // Registration rides the plugin fiber: unregistering with the plugin is
  // handled by the projection registry.
  ...
  async function backoff(agent, turn, step, failure, provider, retry, retryId, delayMs, signal) {
    ...
    agent.session.append('llm/retry', {
      ...
      provider,
      mode: 'normal',
      policyKey: policyKey(codes, delays),
      retry,
  ...
  async function recover({ agent, turn, step, provider, failure, signal }, next) {
    ...
    const key = policyKey(codes, delays)
    const retryState = ctx.sessionProjections.stateOf(agent.session, PROJECTION_KEY)
    // The projected retry count is keyed by (provider, policyKey) exactly
    // like the stock policy's `llmRetry` unit. `stateOf` materializes the
    // fold over the session log on first touch, so restart durability comes
    // from the durable llm/retry events, not from memory.
    const previous = retryState?.[retryStateKey(provider, key)]
